...
* **Definition**: Statistical deviations in agent runtime metrics (rule success rates, token usage, skill invocation counts) that a file-level diff does not capture. Requires real metrics input — bp never synthesizes the numbers.
* **Where Used**: `bp drift baseline --metrics <ndjson>` to record a baseline; `bp drift behavioral --baseline --current` to compare. (File-level drift is separate: `bp verify --level drift` / `bp drift backends`.)
...
* **Definition**: Discovery of blueprint template packages on the public npm registry (ordinary npm packages tagged with `backend:`/`framework:`/`risk:`/`compliance:` keywords). There is no bp-hosted marketplace service; signed pack distribution uses static hosts ([Pack Distribution](pack-distribution.md)).
* **Where Used**: `bp marketplace search`; installs go through `bp template install`.
